...
const Sidebar: React.FC<{ isOpen: boolean; setIsOpen: (open: boolean) => void }> = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <aside className={`${isOpen ? 'w-72' : 'w-24'} bg-white border-r border-neutral-200 transition-all duration-300 flex flex-col sticky top-0 h-screen z-50`}>
      <div className="p-6 flex items-center gap-3 border-b border-neutral-100">
        <div className="w-10 h-10 bg-neutral-900 rounded-xl flex items-center justify-center shrink-0 shadow-lg shadow-neutral-900/20">
      ...
      <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto mt-4">
        {filteredMenuItems.map((item) => (
          ...
          >
            <item.icon className={`w-5 h-5 shrink-0 ${isActive(item.path) ? 'text-white' : 'text-neutral-400 group-hover:text-neutral-900'}`} />
            {isOpen && <span className="font-bold text-sm truncate">{item.label}</span>}
            {isOpen && isActive(item.path) && <ChevronRight className="w-4 h-4 ml-auto opacity-50" />}
          </Link>
        ))}
      </nav>
